...
def MakeInstallerOSX(version, python_versions=[], installdir=None, **kwargs):
    ...
    dist = open("dstroot/distribution.xml", "w")
    dist.write('<?xml version="1.0" encoding="utf-8"?>\n')
    dist.write('<installer-gui-script minSpecVersion="1.000000">\n')
    dist.write('    <title>Panda3D SDK %s</title>\n' % (version))
    dist.write('    <allowed-os-versions>\n')
    dist.write('        <os-version min="10.13"/>\n')
    dist.write('    </allowed-os-versions>\n')
    dist.write('    <options customize="always" allow-external-scripts="no" rootVolumeOnly="false" hostArchitectures="x86_64,arm64"/>\n')
    ...
    for pkg in pkgs:
        ...
        dist.write('    <pkg-ref id="org.panda3d.panda3d.%s.pkg" installKBytes="%d" version="1" auth="Root">file:./%s.pkg</pkg-ref>\n' % (pkg, size, pkg))
    dist.write('</installer-gui-script>\n')
    dist.close()

    # Build a modern flat distribution package using productbuild, so that it
    # installs correctly on recent macOS versions, including Apple Silicon.
    oscmd('/usr/bin/productbuild --distribution dstroot/distribution.xml'
          ' --package-path dstroot/Panda3D/Panda3D.mpkg/Contents/Packages'
          ' --version %s dstroot/Panda3D/Panda3D.pkg' % (version))

    # Remove the intermediate component packages; only the flat package ships.
    oscmd('rm -rf dstroot/Panda3D/Panda3D.mpkg')
    oscmd('rm -f dstroot/distribution.xml')
