...
if (COMPILER=="GCC"):
    ...
    # ODE: prefer pkg-config ("ode.pc", provided by Homebrew and modern distros).
    # The legacy "ode-config" tool is no longer shipped by Homebrew, so relying on
    # it caused the library search path (-L) to be dropped, breaking the link.
    SmartPkgEnable("ODE",       "ode",       ("ode"), "ode/ode.h")
...
# Only do this when Eigen comes from the bundled thirdparty package; if it is
# provided by a system location (e.g. Homebrew via pkg-config), the thirdparty
# eigen directory will not exist and there is nothing to copy.
if not PkgSkip("EIGEN") and GetTarget() in ("windows", "darwin") and GetThirdpartyDir() \
        and os.path.isdir(GetThirdpartyDir()+'eigen/include/Eigen'):
