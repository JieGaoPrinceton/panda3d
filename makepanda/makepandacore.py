...
def SdkLocatePython(prefer_thirdparty_python=False):
    ...
    if GetTarget() == 'windows':
        ...
        if not os.path.exists(py_fwx):
            # Fall back to the framework of the Python that is running makepanda
            # (e.g. a Homebrew or python.org install located outside the
            # standard framework search paths).
            prefix = getattr(sys, "prefix", "")
            if prefix.endswith("/Python.framework/Versions/" + version) or \
               prefix.endswith("/PythonT.framework/Versions/" + version):
                py_fwx = prefix
